...
public class WxPayServiceWrapper {
     ...
     * 查询订单
     */
    public WxPayQueryResponse queryOrder(WxPayQueryRequest request) throws WxPayException {
        WxPayOrderQueryRequest wxRequest = new WxPayOrderQueryRequest();
        wxRequest.setOutTradeNo(request.getOutTradeNo());
        wxRequest.setTransactionId(request.getTransactionId());

        log.debug("微信订单查询请求: {}", wxRequest);

        WxPayOrderQueryResult result = wxPayService.queryOrder(wxRequest);
        log.debug("微信订单查询响应: {}", result);
        ...
        response.setTransactionId(result.getTransactionId());
        response.setTradeType(result.getTradeType());
        response.setTradeState(result.getTradeState());
        response.setTradeStateDesc(result.getTradeStateDesc());
        response.setTotalFee(result.getTotalFee());
        response.setCashFee(result.getCashFee());
